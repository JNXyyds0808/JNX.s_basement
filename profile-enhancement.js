(()=>{
  ...
  function init(){
    ...
    function render(){
      ...
      if(b){const text=(profile.bio||'').trim();b.textContent=text||'还没有个性签名';b.classList.toggle('jnx-empty',!text);}
      const preview=document.getElementById('jnxSettingsAvatarPreview');
      if(preview){if(profile.avatar_url){preview.style.backgroundImage=`url("${profile.avatar_url}")`;preview.classList.add('jnx-has-avatar');preview.textContent='';}else{preview.style.backgroundImage='';preview.classList.remove('jnx-has-avatar');preview.textContent=fallback(name);}}
    ...
    async function loadProfile(){
      ...
      profile=result.data||null;render();
    }
    ...
    async function saveBio(){
      if(!user)return;const input=document.getElementById('jnxSettingsBio');const hint=document.getElementById('jnxSettingsProfileHint');const bio=(input?.value||'').trim();
      ...
      if(profile)profile.bio=bio;render();if(hint){hint.textContent='已保存 ✓';setTimeout(()=>{if(hint)hint.textContent=''},1800);}
    }

    async function uploadAvatar(event){
      if(!user)return;const file=event.target.files?.[0];if(!file)return;
      ...
      const ext=file.type==='image/jpeg'?'jpg':file.type.split('/')[1].toLowerCase();const path=`${user.id}/avatar.${ext}`;
      ...
      const pub=db.storage.from('avatars').getPublicUrl(path);let url=pub.data?.publicUrl;if(!url)return;url+=`?v=${Date.now()}`;
      const save=await db.from('profiles').update({avatar_url:url}).eq('id',user.id);if(save.error){console.error('Avatar profile save error',save.error);return;}
      if(profile)profile.avatar_url=url;render();
    }

    function start(){
      applyStyles();injectProfileDisplay();injectSettingsEditor();loadProfile().then(heartbeat);setInterval(heartbeat,60000);

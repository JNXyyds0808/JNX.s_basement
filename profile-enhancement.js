...
#profileModal .profile-box > h2,
#profileModal .profile-box > .profile-subtitle,
...
#profileAvatar.jnx-has-avatar {
  color: transparent !important;
}
#jnxProfilePublicInfo {
  display: block;
  text-align: center;
  padding: 0 8px 6px;
}
#jnxProfilePublicName {
  font-size: 23px;
  font-weight: 800;
  line-height: 1.2;
  color: inherit;
}
#jnxProfilePublicUsername {
  margin-top: 5px;
  font-size: 13px;
  color: #8e879c;
}
#jnxProfileBioDisplay {
  margin: 16px auto 0;
  max-width: 310px;
  padding: 12px 14px;
  border-radius: 14px;
...
#jnxProfileBioDisplay.jnx-empty {
  color: #777184;
  font-style: italic;
}
#jnxSettingsProfile {
  margin: 18px 0 4px;
  padding-top: 16px;
  border-top: 1px solid rgba(128, 105, 190, 0.18);
}
.jnx-settings-profile-title {
  font-size: 14px;
  font-weight: 700;
  margin-bottom: 12px;
}
.jnx-settings-avatar-row {
  display: flex;
  align-items: center;
  gap: 12px;
  margin-bottom: 14px;
}
#jnxSettingsAvatarPreview {
  width: 54px;
  height: 54px;
  flex: 0 0 54px;
  border-radius: 50%;
  ...
  overflow: hidden;
  background: linear-gradient(135deg, #8b72d9, #c7b7ff);
  color: #fff;
  font:
    700 22px/1 Arial,
    sans-serif;
...
#jnxSettingsAvatarPreview.jnx-has-avatar {
  color: transparent;
}
.jnx-settings-avatar-button {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  padding: 8px 12px;
  border-radius: 10px;
  border: 1px solid rgba(128, 105, 190, 0.24);
  background: rgba(128, 105, 190, 0.1);
  cursor: pointer;
  font-size: 12px;
}
.jnx-settings-field {
...
#jnxSettingsBio {
  ...
  padding: 10px 11px;
  border-radius: 11px;
  border: 1px solid rgba(128, 105, 190, 0.22);
  background: rgba(255, 255, 255, 0.04);
  color: inherit;
  font: inherit;
  outline: none;
}
#jnxSettingsSaveProfile {
  width: 100%;
  margin-top: 10px;
  padding: 9px 12px;
  border: 0;
...
html.jnxLight #jnxProfileBioDisplay {
  background: #f7f4ff;
  color: #5f596b;
}
html.jnxLight #jnxProfilePublicUsername {
  color: #746d80;
}
html.jnxLight #jnxSettingsProfile {
  border-color: rgba(100, 75, 180, 0.18);
}
html.jnxLight #jnxSettingsBio {
  background: #fff;
  color: #242130;
  border-color: rgba(100, 75, 180, 0.22);
}
html.jnxLight .jnx-settings-avatar-button {
  background: #f4efff;
  color: #31274f;
  border-color: rgba(100, 75, 180, 0.22);
}
...
`;
    ...
    function injectProfileDisplay() {
      ...
      if (!info) {
        ...
        info.innerHTML =
          '<div id="jnxProfilePublicName"></div><div id="jnxProfilePublicUsername"></div><div id="jnxProfileBioDisplay"></div>';
    ...
    function injectSettingsEditor() {
      const box =
        document.querySelector("#settingsModal .settings-box") ||
        document.querySelector('#settingsModal [class*="settings-box"]');
      ...
      document
        .getElementById("jnxSettingsAvatarInput")
        ?.addEventListener("change", uploadAvatar);
      document
        .getElementById("jnxSettingsSaveProfile")
        ?.addEventListener("click", saveBio);
    }
    ...
    function render() {
      ...
      if (avatar) {
        if (profile.avatar_url) {
          ...
          avatar.classList.add("jnx-has-avatar");
          avatar.textContent = "";
        } else {
          avatar.style.backgroundImage = "";
          avatar.classList.remove("jnx-has-avatar");
          avatar.textContent = fallback(name);
        }
      }
      const n = document.getElementById("jnxProfilePublicName");
      if (n) n.textContent = name;
      const u = document.getElementById("jnxProfilePublicUsername");
      if (u) u.textContent = username ? `@${username}` : "";
      const b = document.getElementById("jnxProfileBioDisplay");
      if (b) {
        const text = (profile.bio || "").trim();
        b.textContent = text || "还没有个性签名";
        b.classList.toggle("jnx-empty", !text);
      }
    ...
    async function loadProfile() {
      ...
      const result = await db
        ...
        .maybeSingle();
      if (result.error) {
        console.error("Profile enhancement load error", result.error);
        return;
      }
    ...
    async function uploadAvatar(event) {
      ...
      const upload = await db.storage.from("avatars").upload(path, file, {
        upsert: true,
      ...
      const save = await db
        ...
        .eq("id", user.id);
      if (save.error) {
        console.error("Avatar profile save error", save.error);
        return;
      }
    ...
    function start() {
      ...
      document
        ...
        ?.addEventListener("click", () => setTimeout(render, 30));

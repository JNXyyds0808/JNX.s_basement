  ...
  function init() {
    ...
    function getTotalBadge() {
      ...
      if (!badge) {
        ...
        badge.style.cssText =
          "display:none;position:absolute;top:-7px;right:-7px;min-width:18px;height:18px;padding:0 5px;box-sizing:border-box;border-radius:999px;background:#e53935;color:#fff;font:700 11px/18px Arial,sans-serif;text-align:center;z-index:20;pointer-events:none";
    ...
    function ensureUserBadge(button) {
      ...
      if (!badge) {
        ...
        badge.style.cssText =
          "display:none;margin-left:8px;flex:0 0 auto;min-width:17px;height:17px;padding:0 4px;box-sizing:border-box;border-radius:999px;background:#e53935;color:#fff;font:700 10px/17px Arial,sans-serif;text-align:center;vertical-align:middle;transform:none";
        button.appendChild(badge);
      }
      return badge;
    }
    ...
    async function refresh() {
      ...
      let total = 0;
      for (const m of members.data || [])
        total += await unreadForConversation(m.conversation_id);
      setTotalBadge(total);
    ...
    async function refreshUserBadges() {
      ...
      for (const button of buttons) {
        const username = (
          button.querySelector(".jnx-chat-user-username")?.textContent || ""
        )
          .replace(/^@/, "")
          .trim();
    ...
    db.channel("jnx-private-unread")
      ...
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "private_message_reads" },
        (payload) => {

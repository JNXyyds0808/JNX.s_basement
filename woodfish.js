(function () {
  ...
  const PAT_IMAGE =
    "fe31af823d2d7b25d31003b300a0afbecd5f084914cd19-Qgon8B_fw658.png";
  let started = false;
  function init() {
    if (started) return;
...
#jnxGameLauncher {
  ...
  padding: 15px 18px;
  border: 1px solid rgba(133, 133, 255, 0.35);
  border-radius: 16px;
...
#jnxGameLauncher:hover {
  background: rgba(133, 133, 255, 0.22);
  border-color: rgba(150, 150, 255, 0.55);
}
#jnxGameLauncher span {
  font-weight: 700;
  color: #fff;
}
#jnxGameLauncher small {
  color: #c7c7e8;
}
#jnxGameOverlay {
  display: none;
...
.wf-stat {
  flex: 1;
  padding: 11px 8px;
  border: 1px solid rgba(133, 133, 255, 0.22);
  border-radius: 14px;
...
.wf-tap.headpat .wf-default-icon {
  display: none;
}
.wf-default-icon {
  display: inline;
...
`;
    ...
    async function load() {
      ...
      const total = await db
        .from("woodfish_totals")
        .select("count")
        .eq("user_id", user.id)
        .maybeSingle();
      const sum = await db.from("woodfish_daily").select("count").eq("day", d);
      document.getElementById("wfMine").textContent = mine.data?.count || 0;
      document.getElementById("wfForever").textContent = total.data?.count || 0;
      document.getElementById("wfToday").textContent = (sum.data || []).reduce(
      ...
      const rank = await db
        ...
        .limit(10);
      const ids = (rank.data || []).map((x) => x.user_id);
      let names = {};
      if (ids.length) {
        const p = await db
          .from("profiles")
          ...
          .in("id", ids);
        (p.data || []).forEach(
          (x) => (names[x.id] = x.display_name || x.username),
        );
      }
      document.getElementById("wfRank").innerHTML =
        (rank.data || [])
          .map(
            (r, i) =>
              `<div class="wf-row"><span>${i + 1}. ${names[r.user_id] || "用户"}</span><b>${r.count}</b></div>`,
          )
          .join("") ||
        '<div style="opacity:.7;color:#c9c9df">今天还没有人敲木鱼</div>';
    }
    ...
    headpatButton.addEventListener("click", () => {
      headpatMode = !headpatMode;
      localStorage.setItem("jnxWoodfishHeadpatMode", headpatMode ? "1" : "0");
      renderHeadpat();
    });
    renderHeadpat();

(function(){
  ...
  function init(){
    ...
    style.textContent=`#jnxWoodfish{max-width:520px;margin:24px auto;padding:22px;border-radius:22px;background:var(--card-bg,#fff);box-shadow:0 8px 30px rgba(0,0,0,.08);text-align:center}#jnxWoodfish h2{margin:0 0 16px}.wf-stats{display:flex;gap:10px;margin:14px 0}.wf-stat{flex:1;padding:10px;border-radius:14px;background:rgba(128,128,128,.1)}.wf-num{display:block;font-size:24px;font-weight:700}.wf-label{font-size:12px;opacity:.65}.wf-tap{width:150px;height:150px;border-radius:50%;border:0;background:#d8a15d;box-shadow:0 8px 0 #a87338,0 14px 25px rgba(0,0,0,.16);font-size:52px;cursor:pointer;touch-action:manipulation;user-select:none}.wf-tap:active{transform:translateY(6px);box-shadow:0 2px 0 #a87338,0 7px 15px rgba(0,0,0,.12)}.wf-rank{margin-top:20px;text-align:left}.wf-row{display:flex;justify-content:space-between;padding:9px 10px;border-radius:10px}.wf-row:nth-child(1){background:rgba(255,215,0,.18)}.wf-row:nth-child(2){background:rgba(180,180,180,.15)}.wf-row:nth-child(3){background:rgba(180,120,60,.12)}`;
    ...
    async function load(){
      ...
      const sum=await db.from('woodfish_daily').select('count');
    ...
    document.getElementById('wfTap').addEventListener('click',async()=>{const {data:{user}}=await db.auth.getUser();if(!user)return;await db.rpc('tap_woodfish');await load()});

(function () {
  ...
  async function init() {
    ...
    async function getUser() {
      ...
      window.__jnxCurrentUserId = currentUser?.id || null;
      await refreshBadge();
    }
    await getUser();
    ...
    db.channel("jnx-mentions")
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "chat_mentions" },
        async (payload) => {
          if (currentUser && payload.new?.mentioned_user_id === currentUser.id)
            await refreshBadge();
        },
      )
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "chat_mentions" },
        async (payload) => {
          if (currentUser && payload.new?.mentioned_user_id === currentUser.id)
            await refreshBadge();
        },
...
.jnx-private-header h2 {
  color: #fff;
  margin: 0;
  font-size: 20px;
}
.jnx-private-header p {
  color: #777;
  margin: 4px 0 0;
  font-size: 12px;
}
...
`;
    ...
    if (!document.getElementById("jnxPrivateModal")) {
      ...
      modal.innerHTML = `<div class="jnx-private-overlay" id="jnxPrivateOverlay"></div><div class="jnx-private-box"><div class="jnx-private-header"><div><h2 id="jnxPrivateTitle">私人聊天</h2><p>只有你和对方可以看到</p></div><button class="jnx-private-close" id="jnxPrivateClose">×</button></div><div class="jnx-private-messages" id="jnxPrivateMessages"><div class="jnx-private-empty">选择用户开始聊天。</div></div><div class="jnx-private-footer"><textarea class="jnx-private-input" id="jnxPrivateInput" maxlength="2000" placeholder="输入消息..." rows="1"></textarea><button class="jnx-private-send" id="jnxPrivateSend">发送</button></div></div>`;
    ...
    const modal = document.getElementById("jnxPrivateModal"),
      messages = document.getElementById("jnxPrivateMessages"),
      input = document.getElementById("jnxPrivateInput"),
      send = document.getElementById("jnxPrivateSend");
    ...
    function render(rows) {
      ...
      if (!rows.length) {
        messages.innerHTML =
          '<div class="jnx-private-empty">还没有消息，发送第一条吧。</div>';
        return;
      }
      rows.forEach((row) => {
        const item = document.createElement("div");
        item.dataset.privateId = row.id;
        item.className =
          "jnx-private-message" +
          (row.sender_id === window.__jnxCurrentUserId ? " mine" : "");
        const content = document.createElement("div");
        content.className = "jnx-private-content";
        content.textContent = row.content;
        const time = document.createElement("div");
        time.className = "jnx-private-time";
        time.textContent = new Date(row.created_at).toLocaleString("zh-CN");
    ...
    async function findOrCreateConversation(otherId) {
      ...
      if (!me) throw new Error("当前登录状态尚未准备好，请稍后再试。");
      const mine = await db
      ...
      const rpc = await db.rpc("create_private_conversation", {
    ...
    async function openPrivate(user) {
      selectedUser = user;
      document.getElementById("jnxPrivateTitle").textContent =
        user.display_name || user.username || "私人聊天";
      ...
      try {
        ...
        privateChannel = db
          .channel("jnx-private-" + conversationId)
          .on(
            "postgres_changes",
            {
              event: "INSERT",
            ...
            (payload) => {
              ...
              const exists = messages.querySelector(
                '[data-private-id="' + payload.new.id + '"]',
              );
              if (!exists) {
          ...
          .subscribe((status, error) => {
            if (error)
              console.error("Private chat realtime error:", status, error);
          });
        setTimeout(() => input.focus(), 100);
      } catch (error) {
        console.error("Private chat open error:", error);
        messages.innerHTML =
          '<div class="jnx-private-empty">私人聊天暂时无法打开。</div>';
      }
    }
    async function sendPrivate() {
      ...
      const senderId = userResult.data?.user?.id || window.__jnxCurrentUserId;
      if (!senderId) {
        alert("登录状态尚未准备好，请刷新后再试。");
        return;
      }
      send.disabled = true;
      const result = await db.from("private_messages").insert({
      ...
      send.disabled = false;
      if (result.error) {
        console.error("Private send error:", result.error);
      ...
      (event) => {
        ...
        if (userButton) {
          ...
          const buttons = Array.from(
            document.querySelectorAll(".jnx-chat-user"),
          );
          const index = buttons.indexOf(userButton);
          if (index >= 0) {
            const display =
              userButton.querySelector(".jnx-chat-user-display")?.textContent ||
              "JNX User";
            const usernameText =
              userButton.querySelector(".jnx-chat-user-username")
                ?.textContent || "";
    ...
    document
      .getElementById("jnxPrivateClose")
      ?.addEventListener("click", closePrivate);
    document
      .getElementById("jnxPrivateOverlay")
      ?.addEventListener("click", closePrivate);
    ...
    db.auth.getUser().then((result) => {
      window.__jnxCurrentUserId = result.data?.user?.id || null;
    });
    db.auth.onAuthStateChange((_event, session) => {
      window.__jnxCurrentUserId = session?.user?.id || null;
    });
  }

...
.jnx-chat-message.mine .jnx-chat-name {
  color: #ddd;
}
.jnx-chat-content {
  color: #fff;
...
`;
  ...
  async function initChat() {
    ...
    async function loadUsers() {
      ...
      usersBox.innerHTML =
        '<div class="jnx-chat-users-empty">加载用户中...</div>';
      const result = await db
        .from("profiles")
        .select("id,username,display_name")
        .order("username", { ascending: true });
      if (result.error) {
        console.error("User list error:", result.error);
        usersBox.innerHTML =
          '<div class="jnx-chat-users-empty">用户列表暂时无法加载。</div>';
        return;
      }
      ...
      if (!users.length) {
        usersBox.innerHTML =
          '<div class="jnx-chat-users-empty">暂时还没有其他用户。</div>';
        return;
      }
      users.forEach((user) => {
        const button = document.createElement("button");
        ...
        avatar.className = "jnx-chat-avatar";
        avatar.textContent = (user.display_name || user.username || "J")
          .charAt(0)
          .toUpperCase();
        const name = document.createElement("span");
        name.className = "jnx-chat-user-name";
        const display = document.createElement("span");
        display.className = "jnx-chat-user-display";
        display.textContent = user.display_name || user.username || "JNX User";
        const username = document.createElement("span");
        username.className = "jnx-chat-user-username";
        username.textContent = "@" + (user.username || "user");
        name.append(display, username);
        button.append(avatar, name);
        button.addEventListener("click", () =>
          alert("私人聊天功能正在开发中。现在这里显示的是 JNX 用户列表。"),
        );
    ...
    function renderMessages(rows) {
      ...
      if (!rows.length) {
        messagesBox.innerHTML =
          '<div class="jnx-chat-empty">还没有消息，成为第一个发言的人吧。</div>';
        return;
      }
      rows.forEach((row) => {
        const item = document.createElement("div");
        item.className =
          "jnx-chat-message" + (row.user_id === currentUser?.id ? " mine" : "");
    ...
    async function loadMessages() {
      ...
      if (result.error) {
        ...
        messagesBox.innerHTML =
          '<div class="jnx-chat-empty">聊天暂时无法加载。</div>';
        return;
      }
    ...
    async function sendMessage() {
      ...
      const result = await db
        .from("chat_messages")
        .insert({ user_id: currentUser.id, display_name: displayName, content })
        .select("id,user_id,display_name,content,created_at")
        .single();
      send.disabled = false;
      if (result.error) {
    ...
    db.channel("jnx-community-chat")
      .on(
        ...
        () => {
          if (currentUser && modal.classList.contains("active")) loadMessages();
        },
      )
  ...
  document.addEventListener("DOMContentLoaded", function () {
    initChat().catch((error) =>
      console.error("JNX chat initialization failed:", error),
    );
  });
})();

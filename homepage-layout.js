(function(){
  function apply(){
    ...
    if(root.dataset.jnxHomepageLayoutApplied==='true')return true;
    const main=document.querySelector('#home main');
    const recent=document.getElementById('recent');
    const title=main&&main.querySelector('h1');
    if(!main||!recent||!title)return false;
    root.dataset.jnxHomepageLayoutApplied='true';
    const style=document.createElement('style');
    style.id='jnxHomepageLayoutStyle';
    style.textContent=`
      ...
      #jnxRecentLauncher strong{display:block;color:#fff!important}
      #jnxRecentLauncher span{display:block;margin-top:5px;color:#c9c9df!important;font-size:.9em}
      ...
      #jnxRecentWindow{position:relative;width:min(760px,94vw);height:min(82dvh,760px);overflow:auto;box-sizing:border-box;padding:28px;border:1px solid rgba(133,133,255,.32);border-radius:24px;background:#171526;color:#fff;box-shadow:0 20px 70px rgba(0,0,0,.5);overscroll-behavior:contain;touch-action:pan-y}
